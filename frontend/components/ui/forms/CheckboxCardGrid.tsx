...
export type CheckboxOption = { label: string; value: number };
...
export default function CheckboxCardGrid({
  ...
  const colClass = columns === 1 ? "grid-cols-1" : columns === 2 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3";
    ...
    <div className={className} aria-describedby={describedBy}>
      <div className={["grid gap-2", colClass].join(" ")}>
        {options.map((opt, i) => {
          ...
          return (
            <label key={`cb:${opt.label}:${i}`} className={[base, focusRing, selectedRing].join(" ")}>
              <input
                type="checkbox"
                className="sr-only"
                checked={checked}
