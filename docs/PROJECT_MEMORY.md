...
- MailerLite is the B1 integration target for newsletter signups, Fit Check result emails, and future resource waitlists. Analytics setup now includes Cloudflare Web Analytics, Google Search Console with `https://fruitfulpin.com/sitemap.xml`, and GA4 Measurement ID `G-E0TLX9V17Q` wired through `apps/fruitful-pin/components/GoogleAnalytics.tsx`. Affiliate URLs remain pending from Susy.
