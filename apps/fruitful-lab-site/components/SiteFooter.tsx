    ...
    <footer className="border-t border-[var(--border)] bg-[var(--navy)]">
      <div className="mx-auto grid max-w-6xl gap-8 px-5 py-10 text-sm text-[var(--muted)] sm:px-8 md:grid-cols-[1.2fr_1fr]">
        <div>
          ...
          <p className="mt-3 max-w-md leading-6 text-white/70">Marketing systems, funnels, email, paid media, AI workflows, and content strategy for brands that need clearer growth paths.</p>
          <p className="mt-4 text-[var(--amber)]">{CONTACT_EMAIL}</p>
          <p className="mt-6 text-white/54">© {new Date().getFullYear()} {SITE_NAME}. All rights reserved.</p>
        </div>
        <nav className="grid grid-cols-2 gap-3 text-white/70 md:justify-self-end" aria-label="Footer">
